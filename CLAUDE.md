# Claude Code Project Instructions — sha2-kotlin

## Project Overview

This is **sha2-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`sha2`](https://crates.io/crates/sha2).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/sha2/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/sha2>
